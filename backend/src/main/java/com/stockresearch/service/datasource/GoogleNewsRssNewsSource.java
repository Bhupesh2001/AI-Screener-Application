 ...
 *
 * No longer @Primary: superseded by IndianApiNewsSource, which parses
 * news bundled inside the same /stock response already fetched for
 * price/fundamentals (zero extra API calls) rather than a separate,
 * rate-limited call to Google. Left in place as a manually-selectable
 * alternative - swap @Primary back here if IndianAPI's news coverage
 * ever proves thinner for a given company.
 */
@Component
